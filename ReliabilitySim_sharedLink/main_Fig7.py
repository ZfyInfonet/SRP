...
bwUsedList = [[0] * time,[0] * time,[0] * time,[0] * time,[0] * time,[0] * time]
...
df_1.to_csv(f'./results/Fig7a_n{node_num}e{edge_p}a{arrive_rate}r{req_num}.csv',
            header=False, index=False, sep=',')
df_2 = pd.DataFrame(dataFaultNum)
df_2.to_csv(f'./results/Fig7b_n{node_num}e{edge_p}a{arrive_rate}r{req_num}.csv',
            header=False, index=False, sep=',')
