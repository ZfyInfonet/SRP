        ...
        for parent in mInst.Parents:
            ...
            if parent.msId not in parentIdList:
                ...
                for pInst in parentInstList:
                    ...
                    if pInst.nodeId != n.nodeId and pInst.nodeId is not None:
                        ...
                        for path in disjoint_paths:
                            if Tools.CheckLinkConstraints(link, path, G, req):
                                ...
                                pathR *= link.R
                                # algorithm 4
                                pathEdges = G.GetEdgeFromPathNodes(path)
                                minEdgeR = 1
                                for edge in pathEdges:
                                    edgeBwSufficientProbability = 1
                                    for edgeLink in edge.backupLinkList:
                                        if len(req.MsList[edgeLink.msLink[1][0]]) < edgeLink.msLink[1][1]:
                                            continue
                                        son = req.MsList[edgeLink.msLink[1][0]][edgeLink.msLink[1][1] - 1]
                                        # print(edge.BW, edgeLink.bwReq + edge.bwUsed + link.bwReq)
                                        if edge.BW < edgeLink.bwReq + edge.bwUsed + link.bwReq:
                                            if son.sigma != 0:
                                                edgeBwSufficientProbability *= son.sigma
                                    if edgeBwSufficientProbability < minEdgeR:
                                        minEdgeR = edgeBwSufficientProbability
        ...
        if parent.msId not in parentIdList:
            ...
            for pInst in parentInstList:
                ...
                if pInst.nodeId == n_max.nodeId:
                    ...
                    for path in nx.node_disjoint_paths(G.topoGraph, mInst.nodeId, pInst.nodeId, cutoff=MaxNum):
                        if Tools.CheckLinkConstraints(link, path, G, req):
                            paths.append(path)
                            # ---Place Link---
                            if pInst.instId == 1 and mInst.instId == 1:
                                for edge in G.GetEdgeFromPathNodes(path):
                                    edge.AddLink(link)
                            else:
                                for edge in G.GetEdgeFromPathNodes(path):
                                    edge.AddBackupLink(link)
            ...
            if son.msId not in sonIdList:
                ...
                for sonInst in sonInstList:
                    ...
                    if sonInst.nodeId == n_max.nodeId:
                        ...
                        for path in nx.node_disjoint_paths(G.topoGraph, mInst.nodeId, sonInst.nodeId, cutoff=MaxNum):
                            if Tools.CheckLinkConstraints(link, path, G, req):
                                paths.append(path)
                                # ---Place Link---
                                for edge in G.GetEdgeFromPathNodes(path):
